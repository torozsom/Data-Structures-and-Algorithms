...
TEST_F(DynamicArrayUnitTest, MoveConstructor) {
    ...
    unsigned int original_capacity = original.getCapacity();
...
TEST_F(DynamicArrayUnitTest, CapacityExpansionOnInsertion) {
    DynamicArray<int> arr;
    unsigned int initial_capacity = arr.getCapacity();
...
TEST_F(DynamicArrayUnitTest, CapacityReductionOnRemoval) {
    ...
    unsigned int expanded_capacity = arr.getCapacity();
    ...
    unsigned int size_before_trigger = arr.getSize();
...
TEST_F(DynamicArrayUnitTest, OperatorBracketConst) {
    int data[] = {10, 20, 30};
    const DynamicArray<int> arr(data, 3);
