...
namespace ui {
/**
 * @brief Constructs an ArrayWidget that visualizes the given array of values.
 *
 ...
 */
ArrayWidget::ArrayWidget(const data_structs::DynamicArray<int>& values,
                         QWidget* parent)
    : QGraphicsView(parent), scene_(new QGraphicsScene(this)), arrow_(nullptr) {
    setScene(scene_);
    ...
    for (std::size_t i = 0; i < values.size(); ++i) {
        auto rect = scene_->addRect(i * CELL_WIDTH, 0, CELL_WIDTH, CELL_HEIGHT,
                                    pen, QBrush(Qt::white));
        cells_.addLast(rect);
 ...
 */
void ArrayWidget::highlightIndex(const std::size_t index) const {
 ...
 */
void ArrayWidget::setArrowPosition(const std::size_t index) const {
 ...
 */
void ArrayWidget::markFound(const std::size_t index) const {
    if (index < cells_.size())
...
void ArrayWidget::markNotFound() const {
    for (auto* cell : cells_)
        cell->setBrush(QBrush(Qt::white));
}
